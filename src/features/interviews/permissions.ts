...
export async function canCreateInterview(){
   ...
   return await  Promise.any([
        ...
        .then(bool => bool || Promise.reject()),

        Promise.all([hasPermission("1_interview"), getUserInterviewCount()])
        .then(([has , c]) => {
...
async function getInterviewCount(userId : string){
    ...
    .from(InterviewTable)
    .innerJoin(JobInfoTable, 
        eq(InterviewTable.jobInfoId, JobInfoTable.id))
